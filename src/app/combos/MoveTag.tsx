...
export default function MoveTag({ moves }: { moves: string[] }) {
  ...
  return (
    <article ref={ref} className="my-0.5 flex">
      <section className="flex items-center gap-1 text-ellipsis rounded-sm bg-blue-50 py-0.5 pl-2 text-xs font-semibold text-blue-600 dark:bg-blue-600/20 dark:hover:bg-blue-900/70 ">
        <div className="text-[6px] text-gray-400 dark:text-gray-500">
          {1 + (moveIndex ?? 0)}
        </div>
        <div className="">{moves.join(' -> ')}</div>
        {moveIndex !== undefined && (
          <div className="cursor-pointer  px-1 py-1 hover:bg-blue-300/30 hover:dark:bg-blue-400/20">
